...
# ЛОКАТОРЫ. (Локаторы элементов, которые находятся на странице авторизации)
    ...
    title_element = 'title' # Локато на странице указывающий на класс title.
...
# МЕТОДЫ. (Метод, содержащий список ДЕИИСТВИЙ, как шагов.)
    def authorization_user(self):
        ...
        WebDriverWait(self.driver, 60).until(EC.url_to_be(self.url))
        self.input_username('standard_user')# Вызов метода ввода данных о логине стандартного пользователя.
        self.input_password('secret_sauce')  # Вызов метода ввода данных пароля для стандартного пользователя.
