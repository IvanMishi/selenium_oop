...
"""
Базовый класс для реализации Page Object Model (POM)
Принципы ООП:
- Абстракция: Класс предоставляет абстрактные методы для взаимодействия с веб-страницей
- Инкапсуляция: Логика работы с драйвером и вспомогательные методы скрыты внутри класса
- Наследование: Другие классы страниц наследуют общую функциональность
- Полиморфизм: Методы могут быть переопределены в дочерних классах при необходимости
"""
...
class Base(): # Базовый класс в котором хранится драйвер, он будет родительским, для всех наших последующих страниц Pages
    ...
    def __init__(self, driver): # Передает драйвер который хранит экземпляр драйвера chrome
        ...
        self.fake = Faker( "ru_Ru")  # Создаёт экземпляр класса Faker, указывая, что хотим генерировать данные на (ru_Ru - русский язык)
